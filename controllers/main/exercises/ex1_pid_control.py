...
class quadrotor_controller():
    def __init__(self, exp_num):
        ...
        gains = {
                    "P_pos_z": 3.0,     "I_pos_z": 0.0,     "D_pos_z": 2.0,
                    "P_pos_xy": 2.2,    "I_pos_xy": 0.0,    "D_pos_xy": 1.0,
                    "P_vel_z": 8.5,     "I_vel_z": 0.1,     "D_vel_z": 1.75,
                    "P_vel_xy": 0.75,    "I_vel_xy": 0.01,    "D_vel_xy": 0.01,
    ...
    def setpoint_to_pwm(self, dt, setpoint, sensor_data):
        ...
        # Calculate rotation
        R_current = R.from_euler("ZYX", [sensor_data["yaw"], sensor_data["pitch"], sensor_data["roll"]])
        R_body_to_inertial = R_current.as_matrix()   # Rotation from body to inertial frame
        R_inertial_to_body = R_current.as_matrix().T   # Inverse (transpose for rotation matrices)
